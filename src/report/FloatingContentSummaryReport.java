...
class LifetimeSeries
{
	...
	/*
	 * adjust(), incr(), and decr() operate continuously and fill gaps in the time series with copies of the previous value. This only makes sense for monitoring the
	 * #message copies per individual message when time is monotonically increasing.
	 */
...
/**
 * Reports summaries for each Floating Content Message simulation (SUM) and diverse aggregates (TAB): per message size, ttl, a, and r. In the end, individual messages
 * (MSG) are listed.
 */
public class FloatingContentSummaryReport extends Report implements MessageListener
{
	public static final String HEADER = "# FloatingContentMessageSummaryReport";
	...
	private String index(Message m)
	{
		return m.getSize() + "-" + ((Double) m.getProperty(FloatingContentRouter.FC_TTL_VAL)).intValue() + "-"
				+ ((Double) m.getProperty(FloatingContentRouter.FC_R)).intValue() + "-" + ((Double) m.getProperty(FloatingContentRouter.FC_A)).intValue();
	}
	...
	public void messageDeleted(Message m, DTNHost where, boolean dropped)
	{
		...
		/*
		 * do not remove copies when TTL has expired, or we will always get 0 copies -> undo the decrement We have to do the undo because if neither incr nor decr get
		 * called, the previous index value will not be copied within the data structure of the copy counts.
		 */
		if (res_ttl == 0)
		{
	...
	public void done()
	{
		...
		write("SUM\t0\t0\t0\t0\t" + n_msg + "\t" + (double) success / (double) n_msg + "\t" + (double) repl / (double) n_msg + "\t" + (double) abort / (double) n_msg
				+ "\t" + (double) copies / (double) n_msg + "\t" + lifetime.toString("LT", "\t", n_msg) + "\t" + lifecopies.toString("CC", "\t", n_msg));
		for (int s : sizes)
		{
			for (int t : ttls)
			{
				for (int r : rs)
				{
					for (int a : as)
					{
						...
						if (me == null)
						{
							...
							tmp_count = me.count;
						write("TAB\t" + s + "\t" + t + "\t" + r + "\t" + a + "\t" + me.count + "\t" + (double) me.success / (double) tmp_count + "\t"
								+ (double) me.repl / (double) tmp_count + "\t" + (double) me.abort / (double) tmp_count + "\t" + (double) me.copies / (double) tmp_count
