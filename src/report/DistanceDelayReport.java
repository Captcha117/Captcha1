...
/**
 * Report for how far apart the nodes were when the message was sent and how long time & how many hops it took to deliver it. Only messages created after the warm up
 * period are counted. If message is not delivered, its delivery time & hop count are reported as -1
 */
public class DistanceDelayReport extends Report implements MessageListener
{
	...
	public void newMessage(Message m)
	{
		...
		this.creationInfos.put(m.getId(), new InfoTuple(getSimTime(), m.getFrom().getLocation().clone(), m.getTo().getLocation().clone()));
	}
	 ...
	 *            Id of the message
	 * @param startDistance
	 *            Distance of the nodes when the message was creted
	 * @param time
	 *            Time it took for the message to be delivered
	 * @param hopCount
	 *            The amount of hops it took to deliver the message
	 */
