...
/**
 * Abstract superclass for all reports. All settings defined in this class can be used for all Report classes. Some reports don't implement intervalled reports
 * ({@link #INTERVAL_SETTING}) and will ignore that setting. Most of the reports implement warm up feature ({@link #WARMUP_S}) but the implementations are always report
 * specific.
 */
public abstract class Report
{
	...
	/**
	 * Precision of formatted double values - setting id ({@value}). Defines the amount of decimals shown in formatted double values. Default value is
	 * {@value #DEF_PRECISION}.
	 */
	...
	/**
	 * The default output directory of reports (can be overridden per report with {@link Report#OUTPUT_SETTING}) -setting id ({@value})
	 */
	public static final String REPORTDIR_SETTING = "Report.reportDir";
	/**
	 * Warm up period -setting id ({@value}). Defines how many seconds from the beginning of the simulation should not be included in the reports. Implementation of the
	 * feature is report specific, so check out the respective report classes for details. Default is 0. Must be a positive integer or 0.
	 */
	...
	/**
	 * Constructor. Looks for a className.output setting in the Settings and if such is found, uses that as the output file name. Otherwise scenarioname_classname.txt is
	...
	public Report()
	{
		...
		scenarioName = settings.valueFillString(settings.getSetting(SimScenario.SCENARIO_NS + "." + SimScenario.NAME_S));
	...
	/**
	 * Initializes the report output. Method is called in the beginning of every new report file. Subclasses must call this method first in their own implementations of
	 * init().
	 */
	protected void init()
	{
	...
	/**
	 * This method should be called before every new (complete) event the report logs. If the report has no meaningful use for multiple reports, the call can be omitted
	 * (then only single output file will be generated)
	 */
	...
	/**
	 * Formats a double value according to current precision setting (see {@link #PRECISION_SETTING}) and returns it in a string.
	 * 
	 * @param value
	 *            The value to format
	...
	/**
	 * Returns a Settings object initialized for the report class' name space that uses {@value REPORT_NS} as the secondary name space.
	 * 
