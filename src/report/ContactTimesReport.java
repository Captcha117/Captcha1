...
/**
 * Reports the node contact time (i.e., how long they were in the range of each other) distribution. Report file contains the count of connections that lasted for certain
 * amount of time. Syntax:<br>
...
public class ContactTimesReport extends Report implements ConnectionListener
{
	...
	/**
	 * Granularity -setting id ({@value}). Defines how many simulated seconds are grouped in one reported interval.
	 */
	...
	protected void addConnection(DTNHost host1, DTNHost host2)
	{
		...
		assert !connections.containsKey(ci) : "Already contained " + " a connection of " + host1 + " and " + host2;
	...
	protected void increaseTimeCount(double time)
	{
		...
		{
			/*
			 * if biggest index so far, fill array with nulls up to index+2 to keep the last time count always zero
			 */
		...
		if (curValue == null)
		{ // no value found -> put the first
			this.nrofContacts.set(index, 1);
		}
		else
		{ // value found -> increase the number by one
	...
	protected class ConnectionInfo
	{
		...
		/**
		 * Should be called when the connection ended to record the time. Otherwise {@link #getConnectionTime()} will use end time as the time of the request.
		 */
		...
		/**
		 * Returns the time that passed between creation of this info and call to {@link #connectionEnd()}. Unless connectionEnd() is called, the difference between start
		 * time and current sim time is returned.
		 * 
		 * @return The amount of simulated seconds passed between creation of this info and calling connectionEnd()
		 */
		...
		{
			return this.h1 + "<->" + this.h2 + " [" + this.startTime + "-" + (this.endTime > 0 ? this.endTime : "n/a") + "]";
		}
	}
}
