...
/**
 * Movement report that generates suitable movement data for ns-2 simulator as described in <A HREF="http://www.isi.edu/nsnam/ns/doc/node174.html">
...
public class MovementNs2Report extends Report implements MovementListener
{
	...
	/**
	 * Constructor. Reads {@link #NODE_ARR_S} and {@link #NS_CMD_S} settings and uses those values as the name of the node array and ns command. If the values aren't
	 * present, default values of <CODE>{@value DEF_NODE_ARRAY}</CODE> and <CODE>{@value DEF_NS_CMD}</CODE> are used.
	 */
	...
	public void newDestination(DTNHost host, Coord dst, double speed)
	{
		...
		write(nsCmd + " at " + time + " \"\\" + nodeArray + "(" + index + ")" + " setdest " + fix(dst.getX()) + " " + fix(dst.getY()) + " " + speed + "\"");
	}

	/**
	 * Fixes and formats coordinate values suitable for Ns2 module. I.e. converts zero-values to {@value EPSILON} and formats values with {@link #COORD_FORMAT}.
	 * 
	 * @param val
	 *            The value to fix
