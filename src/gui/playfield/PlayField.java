...
public class PlayField extends JPanel
{
	 ...
	 */
	public void setUnderlayImage(BufferedImage image, double dx, double dy, double scale, double rotation)
	{
		if (image == null)
		{
			this.underlayImage = null;
	...
	/**
	 * Enables or disables the automatic clearing of overlay graphics. If enabled, overlay graphics are cleared every time a new graphics object is set to be drawn.
	 * 
	 * @param clear
	...
	/**
	 * Draws the play field. To be called by Swing framework or directly if different context than screen is desired
	 * 
	 * @param g
	...
	public void paint(Graphics g)
	{
		...
		g2.clearRect(-PLAYFIELD_OFFSET, -PLAYFIELD_OFFSET, this.getWidth() + PLAYFIELD_OFFSET, this.getHeight() + PLAYFIELD_OFFSET);
	...
	{
		Coord c = loc.clone();
		c.setLocation(PlayFieldGraphic.scale(c.getX()) + PLAYFIELD_OFFSET, PlayFieldGraphic.scale(c.getY()) + PLAYFIELD_OFFSET);
		return c;
	}

	/**
	 * Returns a world location for a given graphical location. Note that there might be inaccuracies because of rounding.
	 * 
	 * @param loc
	 *            The location to convert
	...
	{
		Coord c = loc.clone();
		c.setLocation(PlayFieldGraphic.invScale(c.getX() - PLAYFIELD_OFFSET), PlayFieldGraphic.invScale(c.getY() - PLAYFIELD_OFFSET));
		return c;
	}
	...
	{
		Dimension minSize = new Dimension(PlayFieldGraphic.scale(w.getSizeX()), PlayFieldGraphic.scale(w.getSizeY()));
