...
public class NodeGraphic extends PlayFieldGraphic
{
	...
	private void drawBar(Graphics2D g2, Coord loc, int nrof, int col)
	{
		...
		{
			if (i % 2 == 0)
			{ // use different color for every other msg
			...
			g2.fillRect(scale(loc.getX() - BAR_DISPLACEMENT - (BAR_WIDTH * col)), scale(loc.getY() - BAR_DISPLACEMENT - i * BAR_HEIGHT), scale(BAR_WIDTH),
					scale(BAR_HEIGHT));
		}
