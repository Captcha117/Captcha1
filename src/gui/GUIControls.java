...
public class GUIControls extends JPanel implements ActionListener, ChangeListener
{
	...
	private static final String TEXT_SIMTIME = "Simulation time - " + "click to force update, right click to change format";
	...
	/**
	 * GUI update speeds. Negative values -> how many 1/10 seconds to wait between updates. Positive values -> show every Nth update
	 */
	...
	{
		if (step)
		{ // if we want to step, return false once and reset stepping
			step = false;
			return false;
		}
		if (SimClock.getTime() >= this.playUntilTime)
	 ...
	 * 
	 * @param delta
	 *            How much to change the current level (can be negative or positive)
	 */
	...
	private void setPlayUntil()
	{
		...
		{
			JOptionPane.showMessageDialog(gui.getParentFrame(), "Invalid number '" + value + "'", "error", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void takeScreenShot()
	{
		try
		{
			...
			if (retVal == JFileChooser.APPROVE_OPTION)
			{
				...
				BufferedImage i = new BufferedImage(this.pf.getWidth(), this.pf.getHeight(), BufferedImage.TYPE_INT_RGB);
		...
		{
			JOptionPane.showMessageDialog(gui.getParentFrame(), "screenshot failed (problems with output file?)", "Exception", JOptionPane.ERROR_MESSAGE);
		}
	}
