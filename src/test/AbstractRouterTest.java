...
/**
 * Superclass for router tests. Sets up the environment by creating multiple hosts with router set by {@link #setRouterProto(MessageRouter)}
 */
public abstract class AbstractRouterTest extends TestCase
{
	...
	protected void deliverMessage(DTNHost from, DTNHost to, String msgId, int msgSize, boolean firstDelivery)
	{
		assertFalse("MC contained " + mc.getLastType(), mc.next());
		from.update(true);
		to.update(true);
