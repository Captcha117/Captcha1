...
/**
 * Test for selecting Points Of Interest from different node groups. Tests that the probability of getting a node from a specific group is approximately same as the
 * defined probability.
 */
public class PointsOfInterestTest extends TestCase
{
	...
	protected void setUpWith(double[] poiProbs, int rngSeed, int[] okNodes) throws Exception
	{
		super.setUp();
	...
	{
		runTestPoiSelection(new double[]
		{ 0.1, 0.8, 0.05 }, 0);
	}
	...
	public void runTestPoiSelection(double[] poiProbs, int rngSeed) throws Exception
	{
		...
		for (int i = 0; i < poiProbs.length; i++)
		{
			...
			double randomProb = (1 - probSum) * (COORDS_IN_MAP[i].length / (nrofMapNodes * 1.0));
		...
		for (int i = 0; i < nrofHits.length; i++)
		{
			...
			assertEquals("Prob too far for index " + i, expectedProbs[i], prob, DELTA);
		}
	...
	{
		int nrof = 100;
		setUpWith(new double[]
		{ 0.1, 0.1, 0.1 }, 1, new int[]
		{ 1 });
