...
public class BinaryEventsReader implements ExternalEventsReader
{
	 ...
	 * 
	 * @param eventsFile
	 *            The file where the events are read
	 */
	public BinaryEventsReader(File eventsFile)
	{
		...
		{
			throw new SimError("Invalid binary input file for external " + "events:" + eventsFile.getAbsolutePath(), e);
		}
	...
	public List<ExternalEvent> readEvents(int nrof)
	{
		...
		{ // FIXME: quick 'n' dirty exception handling
			throw new SimError(e);
		}
		return events;
	}
	 ...
	 * 
	 * @param fileName
	 *            Path to the file where the events are stored
	 * @param events
	 *            List of events to store
	 * @throws IOException
	 *             if something in storing went wrong
	 */
	public static void storeToBinaryFile(String fileName, List<ExternalEvent> events) throws IOException
	{
