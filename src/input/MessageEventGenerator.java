...
{
	/**
	 * Message size range -setting id ({@value}). Can be either a single value or a range (min, max) of uniformly distributed random values. Defines the message size
	 * (bytes).
	 */
	 ...
	 * inter-message creation interval (seconds).
	 */
	public static final String MESSAGE_INTERVAL_S = "interval";
	/**
	 * Sender/receiver address range -setting id ({@value}). The lower bound is inclusive and upper bound exclusive.
	 */
	public static final String HOST_RANGE_S = "hosts";
	/**
	 * (Optional) receiver address range -setting id ({@value}). If a value for this setting is defined, the destination hosts are selected from this range and the source
	 * hosts from the {@link #HOST_RANGE_S} setting's range. The lower bound is inclusive and upper bound exclusive.
	 */
	public static final String TO_HOST_RANGE_S = "tohosts";

	/**
	 * Message ID prefix -setting id ({@value}). The value must be unique for all message sources, so if you have more than one message generator, use different prefix
	 * for all of them. The random number generator's seed is derived from the prefix, so by changing the prefix, you'll get also a new message sequence.
	 */
	public static final String MESSAGE_ID_PREFIX_S = "prefix";
	/**
	 * Message creation time range -setting id ({@value}). Defines the time range when messages are created. No messages are created before the first and after the second
	 * value. By default, messages are created for the whole simulation time.
	 */
	...
	public MessageEventGenerator(Settings s)
	{
		...
		if (this.sizeRange.length == 1)
		{
			...
			this.sizeRange = new int[]
			{ this.sizeRange[0], this.sizeRange[0] };
		}
		else
		{
		...
		if (this.msgInterval.length == 1)
		{
			...
			{ this.msgInterval[0], this.msgInterval[0] };
		}
		else
		{
		...
		if (this.hostRange[1] - this.hostRange[0] < 2)
		{
			...
			{
				throw new SettingsError("Host range must contain at least two " + "nodes unless toHostRange is defined");
			}
			else if (toHostRange[0] == this.hostRange[0] && toHostRange[1] == this.hostRange[1])
			{
				...
				// infinite loop.
				throw new SettingsError("If to and from host ranges contain" + " only one host, they can't be the equal");
			}
		}
