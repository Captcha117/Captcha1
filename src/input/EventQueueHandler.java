...
/**
 * <p>
 * Handler for managing event queues. Supports two different type of event queues: external event queues and event generator classes. For external event queues, the
 * events are defined in external data file(s) (see e.g. input.StandarEventsReader). Event generator classes define events dynamically. Both type of event queues must
 * implement the input.EventQueue interface.
 * <p>
 * The total number of event queues to load is defined with variable <code>NROF_SETTING</code>, e.g.<br>
 ...
 * <p>
 * External event files are used when the variable <code>PATH_SETTING</code> is used to define the path to the event file and event generator class is loaded when the
 * name of the class is defined with <code>CLASS_SETTING</code>.
 */
