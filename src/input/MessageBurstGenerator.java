...
/**
 * Message creation -external events generator. Creates bursts of messages where every source node (defined with {@link MessageEventGenerator#HOST_RANGE_S}) creates a new
 * message to every destination node (defined with {@link MessageEventGenerator#TO_HOST_RANGE_S})on every interval. The message size, burst times, and inter-burst
 * intervals can be configured like with {@link MessageEventGenerator}.
 * 
...
public class MessageBurstGenerator extends MessageEventGenerator
{
	...
	public ExternalEvent nextEvent()
	{
		...
		if (this.hostRange[0] + nextFromOffset == this.toHostRange[0] + nextToOffset)
		{
