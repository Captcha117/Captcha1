...
/**
 * Message creation -external events generator. Creates one message from source node/nodes (defined with {@link MessageEventGenerator#HOST_RANGE_S}) to all destination
 * nodes (defined with {@link MessageEventGenerator#TO_HOST_RANGE_S}). The message size, first messages time and the intervals between creating messages can be configured
 * like with {@link MessageEventGenerator}. End time is not respected, but messages are created until there's a message for every destination node.
 * 
