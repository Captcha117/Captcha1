...
/**
 * <p>
 * External movement reader for traces that are in path format. Uses two trace files, one for the paths and one for specifying activity times. Nodes will follow the paths
 * in the trace file, and pause between paths. Activity times refer to the periods of time when there is valid trace data about the node.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * Trace and activity files ending in .zip, .gz or .gzip are assumed to be compressed and will be automatically uncompressed during reading. The whole trace is loaded
 * into memory at once.
 * </p>
 * 
 * @author teemuk
...
public class ExternalPathMovementReader
{
	 ...
	 */
	private ExternalPathMovementReader(String traceFilePath, String activityFilePath) throws IOException
	{
		...
		try
		{
			if (traceFilePath.endsWith(".gz") || traceFilePath.endsWith(".zip") || traceFilePath.endsWith(".gzip"))
			{
		...
		/*
		 * Scanner scanner = null; try { scanner = new Scanner(inFile); } catch (FileNotFoundException e) { throw new
		 * SettingsError("Couldn't find external movement input " + "file " + inFile); }
		 */
		...
		while (line != null)
		{
			...
			if (readSize >= printSize)
			{
				...
				readSize = 0;
				System.out
						.println("Processed " + (totalRead / 1024) + "KB out" + " of " + (traceSize / 1024) + "KB (" + Math.round(100.0 * totalRead / traceSize) + "%)");
			}
		...
		try
		{
			if (activityFilePath.endsWith(".gz") || activityFilePath.endsWith(".zip") || activityFilePath.endsWith(".gzip"))
			{
				// Grab the first entry from the zip file
				// TODO: try to find the correct entry based on file name
	...
	/**
	 * Sets normalizing of read values on/off. If on, values returned by {@link #readNextMovements()} are decremented by minimum values of the offsets. Default is on
	 * (normalize).
	 * 
	 * @param normalize
	...
	/**
	 * Get an instance of the reader for the given file path. If the file has already been read previously it will not be read again and instead the previous instance of
	 ...
	 */
	public static ExternalPathMovementReader getInstance(String traceFilePath, String activeFilePath)
	{
		if (!ExternalPathMovementReader.singletons.containsKey(traceFilePath))
		{
			try
			{
				ExternalPathMovementReader.singletons.put(traceFilePath, new ExternalPathMovementReader(traceFilePath, activeFilePath));
			}
			catch (IOException e)
			{
				System.exit(1);
