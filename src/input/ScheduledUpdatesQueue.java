...
public class ScheduledUpdatesQueue implements EventQueue
{
	 ...
	 * 
	 * @param simTime
	 *            The time when the update should happen
	 */
	public void addUpdate(double simTime)
	{
		ExternalEvent ee = new ExternalEvent(simTime);

		if (ee.compareTo(nextEvent) == 0)
		{ // this event is already next
			return;
		}
		else if (this.nextEvent.getTime() > simTime)
		{ // new nextEvent
			putToQueue(this.nextEvent); // put the old nextEvent back to q
			this.nextEvent = ee;
		}
		else
		{ // given event happens later..
			putToQueue(ee);
		}
	}
