 ...
 * <STRONG>Note</STRONG>: Understands only <CODE>LINESTRING</CODE>s and <CODE>MULTILINESTRING</CODE>s. Skips all <CODE>POINT</CODE> data. Other data causes IOException.
 */
public class WKTMapReader extends WKTReader
{
	 ...
	 * 
	 * @param bidi
	 *            If true, all read paths are set bidirectional (i.e. if node A is a neighbor of node B, node B is also a neighbor of node A).
	 */
	...
	private MapNode createOrUpdateNode(Coord c, MapNode previous)
	{
		...
		if (n == null)
		{ // no node in that location -> create new
			n = new MapNode(c);
			nodes.put(c, n);
		}
