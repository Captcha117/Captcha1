 ...
 * Instead of checking for every interface if some of the other interfaces are close enough (this approach obviously doesn't scale) we check only interfaces that are
 * "close enough" to be possibly connected. Being close enough is determined by keeping track of the approximate location of the interfaces by storing them in overlay
 * grid's cells and updating the cell information every time the interfaces move. If two interfaces are in the same cell or in neighboring cells, they have a chance of
 * being close enough for connection. Then only that subset of interfaces is checked for possible connectivity.
 * </P>
 * <P>
 * <strong>Note:</strong> this class does NOT support negative coordinates. Also, it makes sense to normalize the coordinates to start from zero to conserve memory.
 */
public class ConnectivityGrid extends ConnectivityOptimizer
{

	/**
	 * Cell based optimization cell size multiplier -setting id ({@value}). Used in {@link World#OPTIMIZATION_SETTINGS_NS} name space. Single ConnectivityCell's size is
	 * the biggest radio range times this. Larger values save memory and decrease startup time but may result in slower simulation. Default value is
	 * {@link #DEF_CON_CELL_SIZE_MULT}. Smallest accepted value is 1.
	 */
	...
	public static void reset()
	{
		...
		{
			throw new SettingsError("Too small value (" + cellSizeMultiplier + ") for " + World.OPTIMIZATION_SETTINGS_NS + "." + CELL_SIZE_MULT_S);
		}
	}
	 ...
	 * 
	 * @param cellSize
	 *            Cell's edge's length (must be larger than the largest radio coverage's diameter)
	 */
	 ...
	 * 
	 * @param key
	 *            A hash value that separates different interfaces from each other
	 * @param maxRange
	 *            Maximum range used by the radio technology using this connectivity grid.
	...
	private GridCell cellFromCoord(Coord c)
	{
		...
		assert row > 0 && row <= rows && col > 0 && col <= cols : "Location " + c + " is out of world's bounds";
	...
	/**
	 * Returns all interfaces that are "near" (i.e., in neighboring grid cells) and use the same technology and channel as the given interface
	 * 
	 * @param ni
	 *            The interface whose neighboring interfaces are returned
	...
	{
		return getClass().getSimpleName() + " of size " + this.cols + "x" + this.rows + ", cell size=" + this.cellSize;
	}
	...
	public class GridCell
	{
		...
		public void moveInterface(NetworkInterface ni, GridCell to)
		{
			...
			assert removeOk : "interface " + ni + " not found from cell with " + interfaces.toString();
		}
		...
		public String toString()
		{
			return getClass().getSimpleName() + " with " + this.interfaces.size() + " interfaces :" + this.interfaces;
		}
	}
