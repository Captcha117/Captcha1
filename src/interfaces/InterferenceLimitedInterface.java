...
/**
 * A simple Network Interface that provides a variable bit-rate service, where the bit-rate depends on the number of other transmitting stations within range The current
 * transmit speed is updated only if there are ongoing transmissions. The configured transmit speed is the maximum obtainable speed.
 */
public class InterferenceLimitedInterface extends NetworkInterface
{
	...
	public void connect(NetworkInterface anotherInterface)
	{
		if (isScanning() && anotherInterface.getHost().isRadioActive() && isWithinRange(anotherInterface) && !isConnected(anotherInterface) && (this != anotherInterface))
		{
			...
			Connection con = new VBRConnection(this.host, this, anotherInterface.getHost(), anotherInterface);
	...
	public void update()
	{
		...
		for (Connection con : this.connections)
		{
			...
			if (((InterferenceLimitedInterface) con.getOtherInterface(this)).isTransferring() == true)
			{
				numberOfActive++;
			}
		}
		...
		currentTransmitSpeed = (int) Math.floor((double) transmitSpeed / (Math.sqrt((1.0 * numberOfActive) * Math.log(1.0 * numberOfActive))) / ntrans);
