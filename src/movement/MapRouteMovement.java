...
/**
 * Map based movement model that uses predetermined paths within the map area. Nodes using this model (can) stop on every route waypoint and find their way to next
 * waypoint using {@link DijkstraPathFinder}. There can be different type of routes; see {@link #ROUTE_TYPE_S}.
 */
public class MapRouteMovement extends MapBasedMovement implements SwitchableMovement
{
	...
	/**
	 * Per node group setting used for selecting a route's type ({@value}). Integer value from {@link MapRoute} class.
	 */
	public static final String ROUTE_TYPE_S = "routeType";

	/**
	 * Per node group setting for selecting which stop (counting from 0 from the start of the route) should be the first one. By default, or if a negative value is given,
	 ...
	 * 
	 * @param proto
	 *            The MapRouteMovement prototype
	 */
