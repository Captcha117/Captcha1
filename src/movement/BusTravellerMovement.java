...
/**
 * 
 * This class controls the movement of bus travellers. A bus traveller belongs to a bus control system. A bus traveller has a destination and a start location. If the
 * direct path to the destination is longer than the path the node would have to walk if it would take the bus, the node uses the bus. If the destination is not provided,
 * the node will pass a random number of stops determined by Markov chains (defined in settings).
 * 
 * @author Frans Ekman
 *
 */
public class BusTravellerMovement extends MapBasedMovement implements SwitchableMovement, TransportMovement
{
	 ...
	 * 
	 * @param nextPath
	 *            The next path the bus is going to take
	 */
	public void enterBus(Path nextPath)
	{
		...
		if (!cbtd.continueTrip())
		{
			...
			this.nextPath = null;
			/*
			 * It might decide not to start walking somewhere and wait for the next bus
			 */
	...
	/**
	 * Small class to help nodes decide if they should continue the bus trip. Keeps the state of nodes, i.e. how many stops they have passed so far. Markov chain
	 * probabilities for the decisions.
	 * 
	...
	public void setNextRoute(Coord nodeLocation, Coord nodeDestination)
	{
		...
		Coord closestToDestination = getClosestCoordinate(allStops, nodeDestination);

		// Check if it is shorter to walk than take the bus
		double directDistance = nodeLocation.distance(nodeDestination);
		double busDistance = nodeLocation.distance(closestToNode) + nodeDestination.distance(closestToDestination);
