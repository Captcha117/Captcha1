...
public class EveningActivityControlSystem
{
	 ...
	 * 
	 * @param eveningMovement
	 *            activity movement
	 */
	public void addEveningActivityNode(EveningActivityMovement eveningMovement)
	{
		eveningActivityNodes.put(new Integer(eveningMovement.getID()), eveningMovement);
	}
	 ...
	 * 
	 * @param eveningActivityNodeID
	 *            unique ID of the node
	 * @return Instructions object
	 */
	public EveningTrip getEveningInstructions(int eveningActivityNodeID)
	{
		EveningActivityMovement eveningMovement = eveningActivityNodes.get(new Integer(eveningActivityNodeID));
		if (eveningMovement != null)
		{
			...
			if (nextTrips[index] == null)
			{
				int nrOfEveningMovementNodes = (int) (eveningMovement.getMinGroupSize()
						+ (double) (eveningMovement.getMaxGroupSize() - eveningMovement.getMinGroupSize()) * rng.nextDouble());
				Coord loc = meetingSpots.get(index).clone();
				nextTrips[index] = new EveningTrip(nrOfEveningMovementNodes, loc);
			}
	...
	/**
	 * Returns a reference to a EveningActivityControlSystem with ID provided as parameter. If a system does not already exist with the requested ID, a new one is
	 * created.
	 * 
	 * @param id
	 *            unique ID of the EveningActivityControlSystem
