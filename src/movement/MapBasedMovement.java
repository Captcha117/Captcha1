...
public class MapBasedMovement extends MovementModel implements SwitchableMovement
{
	...
	/**
	 * Per node group setting for selecting map node types that are OK for this node group to traverse trough. Value must be a comma separated list of integers in range
	 * of [1,31]. Values reference to map file indexes (see {@link #FILE_S}). If setting is not defined, all map nodes are considered OK.
	 */
	...
	private void readOkMapNodeTypes(Settings settings)
	{
		if (settings.contains(MAP_SELECT_S))
		{
			...
			for (int i : okMapNodeTypes)
			{
				...
				{
					throw new SettingsError("Map type selection '" + i + "' is out of range for setting " + settings.getFullPropertyName(MAP_SELECT_S));
				}
				if (i > nrofMapFilesRead)
				{
					throw new SettingsError("Can't use map type selection '" + i + "' for setting " + settings.getFullPropertyName(MAP_SELECT_S) + " because only "
							+ nrofMapFilesRead + " map files are read");
				}
			}
		}
		else
	...
	public Coord getInitialLocation()
	{
		...
		this.lastMapNode = n;
		// return placement;
		// System.out.println(nLocation.getX()+"hhhh"+nLocation.getY());
		// System.out.println(n.getLocation().getX()+"hhhh"+n.getLocation().getY());
		return n2.getLocation().clone();
	}
	...
	/**
	 * Selects and returns a random node that is OK from a list of nodes. Whether node is OK, is determined by the okMapNodeTypes list. If okMapNodeTypes are defined, the
	 * given list <strong>must</strong> contain at least one OK node to prevent infinite looping.
	 * 
	 * @param nodes
	...
	/**
	 * Reads a sim map from location set to the settings, mirrors the map and moves its upper left corner to origo.
	 * 
	...
	private void checkMapConnectedness(List<MapNode> nodes)
	{
		...
		{ // some node couldn't be reached
			...
			throw new SettingsError("SimMap is not fully connected. Only " + visited.size() + " out of " + nodes.size() + " map nodes " + "can be reached from "
					+ firstNode + ". E.g. " + disconnected + " can't be reached");
		}
	}
	...
	private void checkCoordValidity(List<MapNode> nodes)
	{
		...
		for (MapNode n : nodes)
		{
			...
			{
				throw new SettingsError("Map node " + n.getLocation() + " is out of world  bounds " + "(x: 0..." + getMaxX() + " y: 0..." + getMaxY() + ")");
			}
		}
	}
