...
/**
 * 
 * This movement model makes use of several other movement models to simulate movement with daily routines. People wake up in the morning, go to work, go shopping or
 * similar activities in the evening and finally go home to sleep.
 * 
 * @author Frans Ekman
 */
public class WorkingDayMovement extends ExtendedMovementModel
{
	...
	public WorkingDayMovement(Settings settings)
	{
		...
		doEveningActivityProb = settings.getDouble(PROBABILITY_TO_GO_SHOPPING_SETTING);
	...
	public WorkingDayMovement(WorkingDayMovement proto)
	{
		...
		eveningActivityMovement = new EveningActivityMovement(proto.eveningActivityMovement);
	...
	public boolean newOrders()
	{
		...
		case WORK_MODE:
			if (workerMM.isReady())
			{
				...
				{
					movementUsedForTransfers.setNextRoute(workerMM.getOfficeLocation(), eveningActivityMovement.getShoppingLocationAndGetReady());
					mode = BUS_TO_EVENING_ACTIVITY_MODE;
				}
				else
				{
					movementUsedForTransfers.setNextRoute(workerMM.getOfficeLocation(), homeMM.getHomeLocation());
		...
		case HOME_MODE:
			...
			{
				setCurrentMovementModel(movementUsedForTransfers);
				movementUsedForTransfers.setNextRoute(homeMM.getHomeLocation(), workerMM.getOfficeLocation());
		...
		case EVENING_ACTIVITY_MODE:
			...
			{
				setCurrentMovementModel(movementUsedForTransfers);
				movementUsedForTransfers.setNextRoute(eveningActivityMovement.getLastLocation(), homeMM.getHomeLocation());
				mode = BUS_TO_HOME_MODE;
			}
			break;
