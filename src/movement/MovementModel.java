...
/**
 * <P>
 * Superclass for all movement models. All subclasses must contain at least a constructor with one {@link Settings} parameter and also a copy-constructor. They must also
 * implement the {@link #replicate()} method, which should return an instance of the movement model class with same parameters as the object (immutable fields can be
 * shared, but mutable fields must be copied).
 * </P>
 * <P>
...
public abstract class MovementModel
{
	...
	private static void checkMinAndMaxSetting(String name, double min, double max)
	{
		if (min < 0 || max < 0)
		{
			throw new SimError("MovementModel." + name + " (in Settings)" + " has a value less than zero (" + min + ", " + max + ")");
		}
		if (min > max)
		{
			throw new SimError("MovementModel." + name + " (in Settings)" + " min is bigger than max (" + min + ", " + max + ")");
		}
	}
	 ...
	 * 
	 * @param mm
	 *            The MovementModel prototype to base the new object to
	 */
	...
	/**
	 * Generates and returns a suitable waiting time at the end of a path. (i.e. random variable whose value is between min and max of the {@link #WAIT_TIME} setting).
	 * 
	...
	/**
	 * Returns a new path by this movement model or null if no new path could be constructed at the moment (node should wait where it is). A new path should not be
	 * requested before the destination of the previous path has been reached.
	 * 
	...
	/**
	 * Returns a sim time when the next path is available. This implementation returns a random time in future that is {@link #WAIT_TIME} from now.
	 * 
	 ...
	 * 
	 * @return A new path or null edit by spyang
	 */
	public Path getPathByDestination(int origin, int destination)
	{
		return null;
	}

	public Double cost(DTNHost tempHost, int orig, int des)
	{
