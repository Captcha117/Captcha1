...
/**
 * A Class to model movement when people are out shopping or doing other activities with friends. If the node happens to be at some other location than the place where
 * the shopping starts (where it meets its friends), it first travels to the destination along the shortest path.
 * 
 * @author Frans Ekman
 */
		...
		else if (mode == EVENING_ACTIVITY_MODE)
		{
			...
			if (trip.allMembersPresent())
			{
				...
				if (path == null)
				{
					...
					trip.setPath(path);
					double waitTimeAtEnd = (maxWaitTime - minWaitTime) * rng.nextDouble() + minWaitTime;
