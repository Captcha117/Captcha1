...
/**
 * A Class to model movement at home. If the node happens to be at some other location than its home, it first walks the shortest path home location and then stays there
 * until morning. A node has only one home
 * 
 * @author Frans Ekman
 */
public class HomeActivityMovement extends MapBasedMovement implements SwitchableMovement
{
	...
	public HomeActivityMovement(Settings settings)
	{
		...
		{
			timeDifference = (int) Math.min(Math.max((rng.nextGaussian() * timeDiffSTD), -DAY_LENGTH / 2), DAY_LENGTH / 2);
		}
	}
	...
	protected double generateWaitTime()
	{
		...
		{
			return DAY_LENGTH - ((SimClock.getIntTime() + DAY_LENGTH + timeDifference) % DAY_LENGTH);
		}
		else
		{
			return 0;
