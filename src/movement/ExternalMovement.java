...
public class ExternalMovement extends MovementModel
{
	 ...
	 * 
	 * @param mm
	 *            The movement model to copy from
	 */
	private ExternalMovement(MovementModel mm)
	{
		...
		if (initLocations.size() > 0)
		{ // we have location data left
	...
	private static void checkPathNeed()
	{
		...
		if (lastPreloadTime != Double.NaN && SimClock.getTime() >= lastPreloadTime - (samplingInterval * MIN_AHEAD_INTERVALS))
		{
			for (int i = 0; i < nrofPreload && lastPreloadTime != Double.NaN; i++)
			{
				lastPreloadTime = readMorePaths();
			}
		}
	...
	/**
	 * Adds a new location with a time to this model's move pattern. If the node stayed stationary during the update, the current path is put to the queue and a new path
	 * is started once the node starts moving.
	 * 
	 * @param loc
	 *            The location
	 * @param time
	 *            When should the node be there
	 */
	private void addLocation(Coord loc, double time)
	{
		assert samplingInterval > 0 : "Non-positive sampling interval!";

		if (loc.equals(latestLocation))
		{ // node didn't move
			if (latestPath != null)
			{
	...
	public Path getPath()
	{
		...
		if (pathQueue.size() == 0)
		{ // nothing in the queue, return latest
			p = latestPath;
			latestPath = null;
		}
		else
		{ // return first path in the queue
	...
	private static double readMorePaths()
	{
		...
		for (Tuple<String, Coord> t : list)
		{
			...
			if (em != null)
			{ // skip unknown IDs, i.e. IDs not mentioned in...
