...
/**
 * This class models movement at an office. If the node happens to be at some other location than the office, it first walks the shortest path to the office and then
 * stays there until the end of the work day. A node has only works at one office.
 * 
 * @author Frans Ekman
 *
 */
public class OfficeActivityMovement extends MapBasedMovement implements SwitchableMovement
{
	...
	{
		double x_coord = officeLocation.getX() + (0.5 - rng.nextDouble()) * distance;
		if (x_coord > getMaxX())
		{
			x_coord = getMaxX();
		}
		...
		double y_coord = officeLocation.getY() + (0.5 - rng.nextDouble()) * distance;
		if (y_coord > getMaxY())
		{
			y_coord = getMaxY();
		}
	...
	{
		int timeLeft = workDayLength - (SimClock.getIntTime() - startedWorkingTime);
