...
/**
 * A route that consists of map nodes. There can be different kind of routes and the type is determined by the type parameter ({@value #CIRCULAR} or {@value #PINGPONG}).
 */
public class MapRoute
{
	/**
	 * Type of the route ID: circular ({@value}). After reaching the last node on path, the next node is the first node
	 */
	public static final int CIRCULAR = 1;
	/**
	 * Type of the route ID: ping-pong ({@value}). After last node on path, the direction on path is reversed
	 */
	 ...
	 * 
	 * @param stops
	 *            The stops of this route in a list
	 * @param type
	 *            Type of the route (e.g. CIRCULAR or PINGPONG)
	 */
	...
	public MapNode nextStop()
	{
		...
		if (index < 0)
		{ // returned to beginning in ping-pong
	...
	{
		return ((type == CIRCULAR) ? "Circular" : "Ping-pong") + " route with " + getNrofStops() + " stops";
	}
	 ...
	 * 
	 * @param fileName
	 *            name of the file where to read routes
	 * @param type
	 *            Type of the route
	 * @param map
	 *            SimMap where corresponding map nodes are found
	...
	public static List<MapRoute> readRoutes(String fileName, int type, SimMap map)
	{
		...
		{
			throw new SettingsError("Couldn't read MapRoute-data file " + fileName + " (cause: " + ioe.getMessage() + ")");
		}
		...
		for (List<Coord> l : coords)
		{
			...
			for (Coord c : l)
			{
				...
				if (node == null)
				{
					...
					throw new SettingsError("MapRoute in file " + routeFile + " contained invalid coordinate " + c + " orig: " + orig);
				}
				nodes.add(node);
			}
