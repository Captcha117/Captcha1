...
public class PointsOfInterest
{
	 ...
	 *  etc...</CODE><BR>
	 * Sum of probabilities must be less than or equal to one (1.0). If the sum is less than one, chance of getting a random MapPoint is <CODE>1-sum</CODE>.
	 */
	 ...
	 *            Array of map node types that are OK to visit or null if all nodes are OK
	 * @param settings
	...
	/**
	 * Selects a random destination from POIs or all MapNodes. Selecting among POI groups is done by their probabilities. If sum of their probabilities is less than 1.0
	 * and the drawn random probability is bigger than the sum, a random MapNode is selected from the SimMap.
	 * 
	 ...
	 * @return A destination among POIs or all MapNodes
	 */
	public MapNode selectExactDestination(int no)
	{
		// double random = rng.nextDouble();
		int selectno = no;
		// double acc = 0;
	...
	/**
	 * Reads POI selections and their probabilities from given Settings and stores them to <CODE>poiLists</CODE> and <CODE>poiProbs</CODE>.
	 * 
	 * @param s
	 *            The settings file where group specific settings are read
	 * @throws Settings
	 *             error if there was an error while reading the file or some of the settings had invalid value(s).
	 */
	private void readPois(Settings s)
	{
		...
		{
			throw new SettingsError("Invalid amount of POI selection-" + "probability values (" + groupPois.length + "). Must be " + "divisable by 2 in " + fqSetting);
		}
		...
		for (int i = 0; i < groupPois.length - 1; i += 2)
		{
			...
			{
				throw new SettingsError("Invalid probability value (" + prob + ") for POI at index " + index + " in " + fqSetting);
			}
		...
		if (probSum > 1.0)
		{
			throw new SettingsError("Sum of POI probabilities (" + String.format("%.2f", probSum) + ") exceeds 1.0 in " + fqSetting);
		}

	}

	/**
	 * Reads POIs from a file <CODE>{@value POI_FILE_S} + index</CODE> defined in Settings' namespace {@value POI_NS}.
	 * 
	 * @param index
	 ...
	 * @throws Settings
	 *             error if there was an error while reading the file or some coordinate in POI-file didn't match any MapNode in the SimMap
	 */
	private List<MapNode> readPoisOf(int index, Coord offset)
	{
		...
		{
			throw new SettingsError("Couldn't read POI-data from file '" + poiFile + "' defined in setting " + fileSettings.getFullPropertyName(POI_FILE_S + index)
		...
		{
			if (map.isMirrored())
			{ // mirror POIs if map data is also mirrored
			...
			if (node != null)
			{
				...
				{
					throw new SettingsError("POI " + node + " from file " + poiFile + " is on a part of the map that is not " + "allowed for this movement model");
				}
				nodes.add(node);
			}
			else
			{
				throw new SettingsError("No MapNode in SimMap at location " + c + " (after translation) from file " + poiFile);
			}
		}
