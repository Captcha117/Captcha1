...
public abstract class MessageRouter
{
	...
	/**
	 * Message/fragment sending queue type -setting id ({@value}). This setting affects the order the messages and fragments are sent if the routing protocol doesn't
	 * define any particular order (e.g, if more than one message can be sent directly to the final recipient). Valid values are<BR>
	 * <UL>
	...
	/*
	 * Return values when asking to start a transmission: RCV_OK (0) means that the host accepts the message and transfer started, values < 0 mean that the receiving host
	 * will not accept this particular message (right now), values > 0 mean the host will not right now accept any message. Values in the range [-100, 100] are reserved
	 * for general return values (and specified here), values beyond that are free for use in implementation specific cases
	 */
	...
	/**
	 * Constructor. Creates a new message router based on the settings in the given Settings object. Size of the message buffer is read from {@link #B_SIZE_S} setting.
	...
	/**
	 * Returns <code>true</code> if the message has been blacklisted. Messages get blacklisted when an application running on the node wants to drop it. This ensures the
	 * peer doesn't try to constantly send the same message to this node, just to get dropped by an application every time.
	 * 
	 * @param id
	 *            id of the message
	...
	/**
	 * Returns a reference to the messages of this router in collection. <b>Note:</b> If there's a chance that some message(s) from the collection could be deleted (or
	 * added) while iterating through the collection, a copy of the collection should be made to avoid concurrent modification exceptions.
	 * 
	...
	/**
	 * Returns the amount of free space in the buffer. May return a negative value if there are more messages in the buffer than should fit there (because of creating new
	 * messages).
	 * 
	 ...
	 * @return Value zero if the node accepted the message (RCV_OK), value less than zero if node rejected the message (e.g. DENIED_OLD), value bigger than zero if the
	 *         other node should try later (e.g. TRY_LATER_BUSY).
	 */
	...
	/**
	 * This method should be called (on the receiving host) after a message was successfully transferred. The transferred message is put to the message buffer unless this
	 * host is the final recipient of the message.
	 * 
	 * @param id
	 ...
	 *            The message to add
	 * @param newMessage
	 *            If true, message listeners are informed about a new message, if false, nothing is informed. edit by spyang
	 */
	 ...
	 * @param drop
	 *            If the message is dropped (e.g. because of full buffer) this should be set to true. False value indicates e.g. remove of message because it was
	...
	/**
	 * Sorts/shuffles the given list according to the current sending queue mode. The list can contain either Message or Tuple<Message, Connection> objects. Other objects
	 * cause error.
	 * 
	 * @param list
