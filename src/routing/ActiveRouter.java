...
{
	/**
	 * Delete delivered messages -setting id ({@value}). Boolean valued. If set to true and final recipient of a message rejects it because it already has it, the message
	 * is deleted from buffer. Default=false.
	 */
	...
	/**
	 * Called when a connection's state changes. If energy modeling is enabled, and a new connection is created to this node, reduces the energy for the device discovery
	 * (scan response) amount
	 * 
	 * @param @con
	 ...
	 * @return A return code similar to {@link MessageRouter#receiveMessage(Message, DTNHost)}, i.e. {@link MessageRouter#RCV_OK} if receiving seems to be OK,
	 *         TRY_LATER_BUSY if router is transferring, DENIED_OLD if the router is already carrying the message or it has been delivered to this router (as final
	 *         recipient), or DENIED_NO_SPACE if the message does not fit into buffer
	 */
	...
	/**
	 * Tries to make room for a new message. Current implementation simply calls {@link #makeRoomForMessage(int)} and ignores the return value. Therefore, if the message
	 * can't fit into buffer, the buffer is only cleared from messages that are not being sent.
	 * 
	 * @param size
	 ...
	 *            If true, excludes message(s) that are being sent from the oldest message check (i.e. if oldest message is being sent, the second oldest message is
	 *            returned)
	...
	/**
	 * Tries to send messages for the connections that are mentioned in the Tuples in the order they are in the list until one of the connections starts transferring or
	 * all tuples have been tried.
	 * 
	 * @param tuples
	 *            The tuples to try
	...
	/**
	 * Goes trough the messages until the other node accepts one for receiving (or doesn't accept any). If a transfer is started, the connection is included in the list
	 * of sending connections.
	 * 
	 * @param con
	...
	/**
	 * Tries to send all given messages to all given connections. Connections are first iterated in the order they are in the list and for every connection, the messages
	 * are tried in the order they are in the list. Once an accepting connection is found, no other connections or messages are tried.
	 * 
	 * @param messages
	...
	/**
	 * Tries to send all messages that this router is carrying to all connections this node has. Messages are ordered using the
	 * {@link MessageRouter#sortByQueueMode(List)}. See {@link #tryMessagesToConnections(List, List)} for sending details.
	 * 
	 * @return The connections that started a transfer or null if no connection accepted a message.
	 */
	...
	/**
	 * Exchanges deliverable (to final recipient) messages between this host and all hosts this host is currently connected to. First all messages from this host are
	 * checked and then all other hosts are asked for messages to this host. If a transfer is started, the search ends.
	 * 
	...
	/**
	 * Checks out all sending connections to finalize the ready ones and abort those whose connection went down. Also drops messages whose TTL <= 0 (checking every one
	 * simulated minute).
	 * 
	...
	/**
	 * Method is called just before a transfer is aborted at {@link #update()} due connection going down. This happens on the sending host. Subclasses that are interested
	 * of the event may want to override this.
	 * 
	 * @param con
