...
/**
 * Implementation of PRoPHET router as described in <I>Probabilistic routing in intermittently connected networks</I> by Anders Lindgren et al.
 * 
 * 
 * This version tries to estimate a good value of protocol parameters from a timescale parameter given by the user, and from the encounters the node sees during
 * simulation. Refer to Karvo and Ott, <I>Time Scales and Delay-Tolerant Routing Protocols</I> Chants, 2008
 * 
 */
public class ProphetRouterWithEstimation extends ActiveRouter
{
	...
	private boolean updateIET(DTNHost host)
	{
		...
		if (meetings.containsKey(host))
		{
			...
			nrofSamples++;
			meanIET = (((double) nrofSamples - 1) / (double) nrofSamples) * meanIET + (1 / (double) nrofSamples) * timeDiff;
	...
	private void updateParams()
	{
		...
		while (cond == false)
		{
			...
			while (bcheck == false)
			{
				...
				pstable = (1 - zeta) / (Math.exp(b * meanIET) - zeta);
				pnzero = Math.exp(-b * meanIET) * (1 - zeta) * ((1 - Math.pow(zeta * Math.exp(-b * meanIET), ntarg - 1)) / (1 - zeta * Math.exp(-b * meanIET)));
	...
	/**
	 * Updates delivery predictions for a host. <CODE>P(a,b) = P(a,b)_old + (1 - P(a,b)_old) * P_INIT</CODE>
	 * 
	 * @param host
	...
	private void updateTransitivePreds(DTNHost host)
	{
		...
		assert otherRouter instanceof ProphetRouterWithEstimation : "PRoPHET only works " + " with other routers of same type";

		double pForHost = getPredFor(host); // P(a,b)
		Map<DTNHost, Double> othersPreds = ((ProphetRouterWithEstimation) otherRouter).getDeliveryPreds();
	...
	/**
	 * Ages all entries in the delivery predictions. <CODE>P(a,b) = P(a,b)_old * (GAMMA ^ k)</CODE>, where k is number of time units that have elapsed since the last time
	 * the metric was aged.
	 * 
	...
	private class TupleComparator implements Comparator<Tuple<Message, Connection>>
	{
		...
		public int compare(Tuple<Message, Connection> tuple1, Tuple<Message, Connection> tuple2)
		{
			...
			double p1 = ((ProphetRouterWithEstimation) tuple1.getValue().getOtherNode(getHost()).getRouter()).getPredFor(tuple1.getKey().getTo());
			// -"- tuple2...
			double p2 = ((ProphetRouterWithEstimation) tuple2.getValue().getOtherNode(getHost()).getRouter()).getPredFor(tuple2.getKey().getTo());
