...
public class ScheduleDijkstra
{
	 ...
	 * 
	 * @param oracle
	 *            The schedule oracle all nodes are OK
	 */
	...
	{
		this.unvisited = new PriorityQueue<Integer>(PQ_INIT_SIZE, new DurationComparator());
	 ...
	 * @param time
	 *            The time when the path starts
	 * @return a shortest path between the source and destination nodes in a list of Integers or an empty list if such path is not available
	 */
	 ...
	 * 
	 * @param n
	 *            The node whose time is set
	 * @param time
	 *            The time when at given node
	 */
	...
	private class TimeMap
	{
		...
		/**
		 * Returns the currently known smallest time one has a path for to the given node. If no time value is found, returns {@link ScheduleDijkstra#INFINITY} as the
		 * value.
		 * 
		 * @param node
		 *            The node whose time is requested
