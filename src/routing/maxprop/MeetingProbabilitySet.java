...
public class MeetingProbabilitySet
{
	 ...
	 * 
	 * @param maxSetSize
	 *            Maximum size of the probability set; when the set is full, smallest values are dropped when new are added
	 */
	...
	public void updateMeetingProbFor(Integer index)
	{
		...
		/*
		 * now the sum of all entries is 1+alpha; normalize to one by dividing all the entries by 1+alpha
		 */
		...
		{
			core.Debug.p("Probsize: " + probs.size() + " dropping " + probs.remove(smallestEntry.getKey()));
		}
	}
	...
	{
		MeetingProbabilitySet replica = new MeetingProbabilitySet(this.maxSetSize, alpha);
