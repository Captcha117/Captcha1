...
/**
 * Implementation of MaxProp router as described in <I>MaxProp: Routing for Vehicle-Based Disruption-Tolerant Networks</I> by John Burgess et al.
 * 
 * but with parameter estimation for finding an alpha based on timescale definition:
 * 
 * Extension of the protocol by adding a parameter alpha (default 1) By new connection, the delivery likelihood is increased by alpha and divided by 1+alpha. Using the
 * default results in the original algorithm. Refer to Karvo and Ott, <I>Time Scales and Delay-Tolerant Routing Protocols</I> Chants, 2008
 * 
 * This version tries to estimate a good value of alpha from a timescale parameter given by the user, and from the encounters the node sees during simulation.
 * 
 * @version 1.0
 */
public class MaxPropRouterWithEstimation extends ActiveRouter
{
	...
	/**
	 * meeting probabilities of all hosts from this host's point of view mapped using host's network address
	 */
	...
	/**
	 * mapping of the current costs for all messages. This should be set to null always when the costs should be updated (a host is met or a new message is received)
	 */
	...
	public void changedConnection(Connection con)
	{
		...
		{ // new connection
			...
			{
				/*
				 * initiator performs all the actions on behalf of the other node too (so that the meeting probs are updated for both before exchanging them)
				 */
				DTNHost otherHost = con.getOtherNode(getHost());
				MessageRouter mRouter = otherHost.getRouter();

				assert mRouter instanceof MaxPropRouterWithEstimation : "MaxProp only works " + " with other routers of same type";
	...
	/**
	 * Updates transitive probability values by replacing the current MeetingProbabilitySets with the values from the given mapping if the given sets have more recent
	 * updates.
	 * 
	 * @param p
	 *            Mapping of the values of the other host
	 */
	...
	protected boolean updateEstimators(DTNHost host)
	{
		...
		if (meetings.containsKey(host))
		{
			...
			nrofSamplesIET++;
			meanIET = (((double) nrofSamplesIET - 1) / (double) nrofSamplesIET) * meanIET + (1 / (double) nrofSamplesIET) * timeDiff;
		...
		if (encounters.containsKey(host))
		{
			...
			nrofSamplesENC++;
			meanENC = (((double) nrofSamplesENC - 1) / (double) nrofSamplesENC) * meanENC + (1 / (double) nrofSamplesENC) * (double) encounterNro;
	...
	protected void updateParam()
	{
		...
		/*
		 * the estimation algorith does not work for timescales shorter than the mean IET - so use defaults
		 */
	...
	/**
	 * Method is called just before a transfer is finalized at {@link ActiveRouter#update()}. MaxProp makes book keeping of the delivered messages so their IDs are
	 * stored.
	 * 
	 * @param con
	 ...
	 * 
	 * @param newValue
	 *            The new value to add to the estimate
	 */
	private void updateTransferredBytesAvg(int newValue)
	{
		...
		{
			if (this.avgSamples[i] > 0)
			{ // only values above zero count
				realCount++;
		...
		{ // no samples or all samples are zero
			this.avgTransferredBytes = 0;
		}
	}

	/**
	 * Returns the next message that should be dropped, according to MaxProp's message ordering scheme (see {@link MaxPropTupleComparator}).
	 * 
	 * @param excludeMsgBeingSent
	 *            If true, excludes message(s) that are being sent from the next-to-be-dropped check (i.e., if next message to drop is being sent, the following message
	 *            is returned)
	...
	/**
	 * Returns the message delivery cost between two hosts from this host's point of view. If there is no path between "from" and "to" host, Double.MAX_VALUE is returned.
	 * Paths are calculated only to hosts that this host has messages to.
	 * 
	 * @param from
	 *            The host where a message is coming from
	 * @param to
	 *            The host where a message would be destined to
	 * @return The cost of the cheapest path to the destination or Double.MAX_VALUE if such a path doesn't exist
	 */
	...
	private Tuple<Message, Connection> tryOtherMessages()
	{
		...
		/*
		 * for all connected hosts that are not transferring at the moment, collect all the messages that could be sent
		 */
		...
		/*
		 * sort the message-connection tuples according to the criteria defined in MaxPropTupleComparator
		 */
	...
	/**
	 * Calculates and returns the current threshold value for the buffer's split based on the average number of bytes transferred per transfer opportunity and the hop
	 * counts of the messages in the buffer. Method is public only to make testing easier.
	 * 
	...
	public int calcThreshold()
	{
		...
		/*
		 * now i points to the first packet that exceeds portion p; the threshold is that packet's hop count + 1 (so that packet and perhaps some more are included in the
		 * priority part)
		 */
	...
	/**
	 * Message comparator for the MaxProp routing module. Messages that have a hop count smaller than the given threshold are given priority and they are ordered by their
	 * hop count. Other messages are ordered by their delivery cost.
	 */
	private class MaxPropComparator implements Comparator<Message>
	{
		...
		/**
		 * Constructor. Assumes that the host where all the costs are calculated from is this router's host.
		 * 
		 * @param treshold
		 *            Messages with the hop count smaller than this value are transferred first (and ordered by the hop count)
		 */
		 ...
		 *            Messages with the hop count smaller than this value are transferred first (and ordered by the hop count)
		 * @param from1
		 *            The host where the cost of msg1 is calculated from
		 * @param from2
		 *            The host where the cost of msg2 is calculated from
		 */
		...
		/**
		 * Compares two messages and returns -1 if the first given message should be first in order, 1 if the second message should be first or 0 if message order can't
		 * be decided. If both messages' hop count is less than the threshold, messages are compared by their hop count (smaller is first). If only other's hop count is
		 * below the threshold, that comes first. If both messages are below the threshold, the one with smaller cost (determined by
		...
		public int compare(Message msg1, Message msg2)
		{
			...
			/*
			 * if one message's hop count is above and the other one's below the threshold, the one below should be sent first
			 */
			...
			/*
			 * if both are below the threshold, one with lower hop count should be sent first
			 */
			if (hopc1 < threshold && hopc2 < threshold)
			{
				return hopc1 - hopc2;
			...
			/*
			 * both messages have more than threshold hops -> cost of the message path is used for ordering
			 */
			...
			{
				/*
				 * if costs are equal, hop count breaks ties. If even hop counts are equal, the queue ordering is used
				 */
				if (hopc1 == hopc2)
				{
	...
	/**
	 * Message-Connection tuple comparator for the MaxProp routing module. Uses {@link MaxPropComparator} on the messages of the tuples setting the "from" host for that
	 * message to be the one in the connection tuple (i.e., path is calculated starting from the host on the other end of the connection).
	 */
	private class MaxPropTupleComparator implements Comparator<Tuple<Message, Connection>>
	{
		...
		/**
		 * Compares two message-connection tuples using the {@link MaxPropComparator#compare(Message, Message)}.
		 */
