...
/**
 * Replication policy options fifo: do nothing special; replicate in the order in which they are found in the buffer rnd: shuffle randomly saf: smallest anchor zone
 * (area) first svf[2]: smallest cylinder (area x volume) first; svf2 uses area, svf just radius stf[2]: smallest total (area x volume x ttl) first; stf2 uses area, stf
 * just radius
 */
...
public class FloatingContentRouter extends ActiveRouter
{
	...
	public void update()
	{
		...
		/*
		 * If we do not have an own notion of a location, we infer our location as the mean of the nodes we are connected to. This can be done in reality if the other
		 * nodes broadcast beacons with their location information.
		 */
		...
		{
			/*
			 * This branch of the code deletes the message upon first encounter of another node outside the anchor zone We loop through the new connections to perform the
			 * evaluation of the probabilistic deletion check once per new connection.
			 */
		...
		{
			/*
			 * find the right connection(s) for each message Messages are replicated to nodes if the target node is within the anchor zone If the other node doesn't know
			 * where it is, we'll pick the position of the local node. If the local node doesn't know its position either, we check if the peer knows its previous
			 * location and choose this. If this isn't known either, we stay passive and don't do anything.
			 *
