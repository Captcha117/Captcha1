...
/**
 * Energy model for routing modules. Handles power use from scanning (device discovery), scan responses, and data transmission. If scanning is done more often than 1/s,
 * constant scanning is assumed (and power consumption does not increase from {@link #scanEnergy} value).
 */
public class EnergyModel implements ModuleCommunicationListener
{
	/**
	 * Initial units of energy -setting id ({@value}). Can be either a single value, or a range of two values. In the latter case, the used value is a uniformly
	...
	/**
	 * Energy update warmup period -setting id ({@value}). Defines the simulation time after which the energy level starts to decrease due to scanning, transmissions,
	 * etc. Default value = 0. If value of "-1" is defined, uses the value from the report warmup setting {@link report.Report#WARMUP_S} from the namespace
	 * {@value report.Report#REPORT_NS}.
	 */
	public static final String WARMUP_S = "energyWarmup";

	/**
	 * {@link ModuleCommunicationBus} identifier for the "current amount of energy left" variable. Value type: double
	 */
	...
	public EnergyModel(Settings s)
	{
		...
		{
			throw new SettingsError(INIT_ENERGY_S + " setting must have " + "either a single value or two comma separated values");
		}
	 ...
	 * 
	 * @param proto
	 *            The model prototype where setting values are copied from
	 */
	 ...
	 * 
	 * @param range
	 *            The min and max values of the range, or if only one value is given, that is used as the energy level
	 */
	...
	/**
	 * Updates the current energy so that the given amount is reduced from it. If the energy level goes below zero, sets the level to zero. Does nothing if the warmup
	 * time has not passed.
	 * 
	 * @param amount
	 *            The amount of energy to reduce
	 */
