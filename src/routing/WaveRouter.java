...
public class WaveRouter extends ActiveRouter
{
	...
	/**
	 * Immunity time -setting id ({@value}). Defines how long time a node will reject incoming messages it has already received
	 */
	public static final String IMMUNITY_S = "immunityTime";
	/**
	 * Custody fraction -setting id ({@value}). Defines how long (compared to immunity time) nodes accept custody for new incoming messages.
	 */
	...
	public RoutingInfo getRoutingInfo()
	{
		...
		{
			RoutingInfo m = new RoutingInfo(id + " until " + String.format("%.2f", recentMessages.get(id) + this.immunityTime));
