...
/**
 * Simple ping application to demonstrate the application support. The application can be configured to send pings with a fixed interval or to only answer to pings it
 * receives. When the application receives a ping it sends a pong message in response.
 * 
 * The corresponding <code>PingAppReporter</code> class can be used to record information about the application behavior.
 * 
 * @see PingAppReporter
 * @author teemuk
 */
public class PingApplication extends Application
{
	...
	public void update(DTNHost host)
	{
		...
		if (curTime - this.lastPing >= this.interval)
		{
			...
			Message m = new Message(host, randomHost(), "ping" + SimClock.getIntTime() + "-" + host.getAddress(), getPingSize());
