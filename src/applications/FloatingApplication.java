...
/**
 * Simple floating application to demonstrate the application support. The application can be configured to send floatings with a fixed interval or to only answer to
 * floatings it receives. When the application receives a floating it sends a pong message in response.
 * 
 * The corresponding <code>FloatingAppReporter</code> class can be used to record information about the application behavior.
 * 
...
public class FloatingApplication extends Application
{
	...
	public FloatingApplication(Settings s)
	{
		...
		/*
		 * at which point in time do we start generating floating content messages
		 */
		...
		/*
		 * messages need to have an unreachable destination or they would get delivered
		 */
		...
		/*
		 * configure the anchor zone: min, max positions, r, a, and the intervals
		 */
		...
		if (s.contains(FLOATING_ANCHOR_MAX))
		{
			...
			anchor_r_max = destination[2] > anchor_r ? destination[2] : anchor_r;
			anchor_a_max = destination[3] > anchor_a ? destination[3] : anchor_a;
		}
		...
		/*
		 * for fixed anchor points, how close does a node have to be to the center
		 */
		...
		/*
		 * determining the flux of nodes into/out of an anchor zone; not used in regular operation
		 */
		...
		if (s.contains(FLOATING_MODE))
		{
			...
			else if (s.getSetting(FLOATING_MODE).equals("variable"))
			{
				...
				throw new core.SettingsError("Unknown mode: " + s.getSetting(FLOATING_MODE));
		}
		...
		if (anchor_max.getX() - anchor.getX() < anchor_gran.getX())
			anchor_gran.setLocation(anchor_max.getX() - anchor.getX(), anchor_gran.getY());
		if (anchor_max.getY() - anchor.getY() < anchor_gran.getY())
			anchor_gran.setLocation(anchor_gran.getX(), anchor_max.getY() - anchor.getY());
	...
	/**
	 * Handles an incoming message. Nothing done in this basic case where messages are only posted, but not replied to. This may serve as a hook to create more complex
	 * application logic.
	 * 
	 * @param msg
	...
	/**
	 * Return the target host (a 'virtual' host) for the floating message This should be a host somewhere at an unreachable position
	 * 
	 * @return host
	 */
	...
	public void update(DTNHost host)
	{
		...
		case MODE_FIXED_ANCHOR:
			...
			if (curTime - lastMessage >= interval)
			{
				...
				if (distance < anchor_r * in_anchor_zone_factor)
				{
					...
					Message m = new Message(host, destinationHost(), "floating" + msg_seq_no++ + "-" + SimClock.getIntTime() + "-" + host.getAddress(), getMessageSize());
		...
		case MODE_VARIABLE_ANCHOR:
			/*
			 * reinterpret the anchor value: Use r as the minimal replication range. Use anchor as the middle point +/- a into X and Y direction defines the area in which
			...
			double r = (anchor_r_max == anchor_r) ? anchor_r
					: (rng_r.nextInt(((int) anchor_r_max - (int) anchor_r) / (int) anchor_r_gran + 1)) * anchor_r_gran + anchor_r;
			...
			if (curTime - lastMessage >= interval && (fcr.getLocationSource() == FloatingContentRouter.LOC_SRC_GPS || fcr.getLastKnownLocation() != null))
			{
				/*
				 * "Snap to grid" yet to be implemented. This would use the anchor_gran values for x and y and make the message snap to the closest coordinate.
				 *
				 * Again, for this check, we use the real location and not the perceived one. The perceived location will be inserted into the floating content message
				 * inside the FloatingContentRouter class, so there is nothing to do here. The only exception is the anchor point.
				 */
				if (host.getLocation().getX() >= anchor.getX() && host.getLocation().getX() <= anchor_max.getX() && host.getLocation().getY() >= anchor.getY()
						&& host.getLocation().getY() <= anchor_max.getY())
				{

					Message m = new Message(host, destinationHost(), "floating" + msg_seq_no++ + "-" + SimClock.getIntTime() + "-" + host.getAddress(), msgsize);
					m.addProperty("type", "floating");
					m.addProperty("r", r);
					m.addProperty("a", a);
