...
abstract public class NetworkInterface implements ModuleCommunicationListener
{
	...
	/**
	 * Activeness offset jitter -setting id ({@value}) The maximum amount of random offset for the offset
	 */
	...
	/**
	 * {@link ModuleCommunicationBus} identifier for the "transmission speed" variable. Value type: integer
	 */
	...
	public NetworkInterface(Settings s)
	{
		...
		// RSU鍜孭V鐨勬帴鍙ｈ繘琛屽綊涓�
		if (this.interfacetype.equals("pvInterface"))
		{
			this.interfacetype = "rsuInterface";
		}
	...
	public void setHost(DTNHost host)
	{
		...
		if (transmitRange > 0)
		{
			optimizer = ConnectivityGrid.ConnectivityGridFactory(this.interfacetype.hashCode(), transmitRange);
	 ...
	 * 
	 * Overload this in a derived class. Check the requirements for the connection to work in the derived class, then call connect(Connection, NetworkInterface) for the
	 * actual connection.
	 * 
	...
	/**
	 * This method is called by the {@link ModuleCommunicationBus} when/if someone changes the scanning interval, transmit speed, or range
	 * 
	 * @param key
	...
	/**
	 * Creates a connection to another host. This method does not do any checks on whether the other node is in range or active (cf. {@link #connect(NetworkInterface)}).
	 * 
	 * @param anotherInterface
	 ...
	 * 
	 * @param anotherInterface
	 *            The other host's network interface to disconnect from this host
	 */
	...
	{
		return "net interface " + this.address + " of " + this.host + ". Connections: " + this.connections;
	}
