...
public class DTN2Manager
{
	...
	/**
	 * Sets up the dtnd connections by parsing the configuration file defined in the <code>DTN2.configFile</code> setting.
	 * 
	 * @param world
	 *            reference to the world that contains the nodes
	 */
	public static void setup(World world)
	{
		...
		while (s != null)
		{
			...
			if (attrs.length == 5 && !s.startsWith("#"))
			{
				...
				DTN2Events.ParserHandler ph = DTN2Manager.events.getParserHandler(nodeID, dtnd_host, console_port);
				p.setListener(ph);
	...
	/**
	 * Sets the <code>DTN2Reporter</code> object used to pass messages from ONE to dtnd. This should be used by the dynamically loaded DTN2Reporter object to allow other
	 * objects get reference to it.
	 * 
	 * @param reporter
	 *            the reporter object to save reference to
	 */
	 ...
	 * 
	 * @param events
	 *            the active events object to use
	 */
	...
	/**
	 * Returns a <code>Collection</code> of <code>DTNHost</code> objects corresponding to the given EID.
	 * 
	 * @param EID
	 *            EID of the host
