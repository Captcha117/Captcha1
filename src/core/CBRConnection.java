...
public class CBRConnection extends Connection
{
	 ...
	 *            Transfer speed of the connection (Bps) when the connection is initiated
	 */
	public CBRConnection(DTNHost fromNode, NetworkInterface fromInterface, DTNHost toNode, NetworkInterface toInterface, int connectionSpeed)
	{
		super(fromNode, fromInterface, toNode, toInterface);
		this.speed = connectionSpeed;
		this.transferDoneTime = 0;

		//System.out.println("ConnectionUp:"+fromNode+"->"+toNode);
	}

	/**
	 * Sets a message that this connection is currently transferring. If message passing is controlled by external events, this method is not needed (but then e.g.
	 * {@link #finalizeTransfer()} and {@link #isMessageTransferred()} will not work either). Only a one message at a time can be transferred using one connection.
	 * 
	 * @param from
	...
	{
		assert this.msgOnFly == null : "Already transferring " + this.msgOnFly + " from " + this.msgFromNode + " to " + this.getOtherNode(this.msgFromNode) + ". Can't "
				+ "start transfer of " + m + " from " + from;

		this.msgFromNode = from;
		Message newMessage = m.replicate();

		// newMessage.updateProperty("transmit_times", (int)newMessage.getProperty("transmit_times")+1); //edited by spyang
	...
	public void abortTransfer()
	{
		...
		getOtherNode(msgFromNode).messageAborted(this.msgOnFly.getId(), msgFromNode, getRemainingByteCount());
		clearMsgOnFly();
		this.transferDoneTime = 0;
	}
	...
	{
		return super.toString() + (isTransferring() ? " until " + String.format("%.2f", this.transferDoneTime) : "");
	}
