...
{
	/**
	 * Time-to-live (TTL) as seconds -setting id ({@value}). Boolean valued. If set to true, the TTL is interpreted as seconds instead of minutes. Default=false.
	 */
	...
	/**
	 * Container for generic message properties. Note that all values stored in the properties should be immutable because only a shallow copy of the properties is made
	 * when replicating messages
	 */
	...
	/**
	 * Creates a new Message contains destination.
	 * 
	 * @param from
	 *            Who the message is (originally) from
	 * @param to
	 ...
	 *            Size of the message (in bytes)
	 * @param destination
	 *            edited by spyang 2017/07/17
	 */
	public Message(DTNHost from, DTNHost to, String id, int size, Coord destination)
	{
		this.from = from;
		this.to = to;
		this.id = id;
	...
	/**
	 * Returns the time to live (in minutes or seconds, depending on the setting {@link #TTL_SECONDS_S}) of the message or Integer.MAX_VALUE if the TTL is infinite.
	...
	/**
	 * Sets the initial TTL (time-to-live) for this message. The initial TTL is the TTL when the original message was created. The current TTL is calculated based on the
	 * time of
	 * 
	 * @param ttl
	...
	/**
	 * Adds a generic property for this message. The key can be any string but it should be such that no other class accidently uses the same value. The value can be any
	 * object but it's good idea to store only immutable objects because when message is replicated, only a shallow copy of the properties is made.
	 * 
	 * @param key
	...
	/**
	 * Updates a value for an existing property. For storing the value first time, {@link #addProperty(String, Object)} should be used which checks for name space
	 * clashes.
	 * 
	 * @param key
	 *            The key which is used to lookup the value
