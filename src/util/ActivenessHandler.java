...
/**
 * Object of this class tell the models when a node belonging to a certain group is active and when not.
 */
public class ActivenessHandler
{
	 ...
	 * </P>
	 * <P>
	 * Defines the simulation time ranges when the node is active. Multiple timer ranges can be concatenated by repeating the sequence. Time limits must be in order and
	 * must not overlap.
	 * </P>
	 * <P>
	 * Example: 100,200,6000,6500<BR>
	 * Here node is active from 100 to 200 and from 6000 to 6500 simulated seconds from the start of the simulation.
	 * </P>
	 */
	 ...
	 * <P>
	 * Example: 2000,500<BR>
	 * Here node is periodically first active for 2000 seconds, followed by 500 seconds of inactiveness, and 2000 seconds of activeness, 500 seconds of inactiveness, etc.
	 * </P>
	 */
	 ...
	 * </P>
	 * <P>
	 * Defines how much the activity periods are offset from sim time 0. Value X means that the first period has been on for X seconds at sim time 0. Default = 0 (i.e.,
	 * first active period starts at 0)
	 * </P>
	 */
	...
	private Queue<TimeRange> parseActiveTimes(Settings s)
	{
		...
		if (s.contains(ACTIVE_TIMES_S))
		{
			...
			if (times.length % 2 != 0)
			{
				throw new SettingsError("Invalid amount of values (" + times.length + ") for setting " + sName + ". Must " + "be divisable by 2");
			}
		}
		else
		{
		...
		for (int i = 0; i < times.length; i += 2)
		{
			...
			if (start > end)
			{
				throw new SettingsError("Start time (" + start + ") is " + " bigger than end time (" + end + ") in setting " + sName);
			}
	...
	public boolean isActive(int offset)
	{
		...
		if (this.curRange.isOut(time))
		{ // time for the next time range
			this.curRange = activeTimes.poll();
			if (curRange == null)
			{
				return false; // out of active times
